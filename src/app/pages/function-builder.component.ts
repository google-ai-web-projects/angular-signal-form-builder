      ...
      <div class="w-64 border-r border-gray-200 bg-gray-50 flex flex-col h-full shrink-0">
         ...
         <div class="flex-1 overflow-y-auto">
            <div class="p-3 border-b border-gray-100 bg-gray-50/50">
               <div class="relative">
                  <mat-icon class="absolute left-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-[18px] w-[18px] h-[18px]">search</mat-icon>
                  <input type="text" [ngModel]="searchTerm()" (ngModelChange)="searchTerm.set($event)" placeholder="Search functions..." class="w-full pl-9 pr-3 py-1.5 bg-white border border-gray-300 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-shadow">
               </div>
            </div>
            @for (func of filteredFunctions(); track func.id) {
               <div 
               ...
               <div class="w-80 lg:w-96 bg-white flex flex-col h-full z-0 overflow-hidden shrink-0 hidden md:flex">
                  ...
                  <div class="flex-1 overflow-y-auto p-4 space-y-4">
                     ...
                     <div class="space-y-3">
                        ...
                        <div>
                           <label class="block text-xs font-medium text-gray-600 mb-1">Mock Form Values (JSON Object)</label>
                           <textarea [ngModel]="testValues()" (ngModelChange)="testValues.set($event)" placeholder='e.g. {"price": 100}' class="w-full h-16 font-mono text-xs p-2 bg-gray-50 border border-gray-200 rounded-md outline-none focus:ring-1 focus:ring-indigo-500" spellcheck="false"></textarea>
                        </div>
...
export class FunctionBuilderComponent {
  ...
  filteredFunctions = computed(() => {
    ...
    if (!term) return fns;
    return fns.filter(f => f.name?.toLowerCase().includes(term) || f.description?.toLowerCase().includes(term));
  });
  ...
  deleteFunction(id: string) {
    if (!confirm('Are you sure you want to delete this function? Any fields using it might break.')) return;
    const currentConfig = this.formBuilder.formConfig();
  ...
  async runTest() {
     ...
     try {
         ...
         this.runError.set(e.message || 'Invalid JSON in form values input.');
         return;
     }
     
     try {
         const functionsMap: Record<string, (...args: any[]) => any> = {};
         this.functions().forEach(fnDef => {
             const pNames = fnDef.isVoid ? [] : (fnDef.parameters || []).map(p => p.name);
             functionsMap[fnDef.name] = (...args: any[]) => {
                 return this.expressionEvaluator.evaluateAsync(fnDef.body, args, pNames, { values: parsedValues, form: { getValue: (key: string) => parsedValues[key], setValue: () => {} } });
             };
         });
         const paramNames = func.isVoid ? [] : func.parameters.map(p => p.name);
         const context = { 
             values: parsedValues, 
             form: { getValue: (key: string) => parsedValues[key], setValue: () => {} },
             functions: functionsMap,
             fns: functionsMap,
             ...functionsMap
         };
         const result = await this.expressionEvaluator.evaluateAsync(func.body, parsedArgs, paramNames, context);
